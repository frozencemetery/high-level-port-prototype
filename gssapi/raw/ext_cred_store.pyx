...
def acquire_cred_from(dict store, Name name, ttl=None,
    ...
    if maj_stat == GSS_S_COMPLETE:
        rc.raw_creds = creds
        return AcquireCredResult(rc, c_create_oid_list(actual_mechs),
...
def add_cred_from(dict store, Creds input_creds,
    ...
    if maj_stat == GSS_S_COMPLETE:
        rc = Creds()
        rc.raw_creds = creds
        return AddCredResult(rc, c_create_oid_list(actual_mechs),
...
def store_cred_into(dict store, Creds creds not None,
    ...
    if maj_stat == GSS_S_COMPLETE:
        ...
        return StoreCredResult(c_create_oid_list(actual_mech_types),
                               py_actual_usage)
    else:
        raise GSSError(maj_stat, min_stat)
